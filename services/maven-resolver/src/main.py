...
async def process_package(cve_id: str, pkg: dict):
    ...
    if redis_client.check_rate_limit(
        f"rate:maven:{pkg_name}", limit=100, window_seconds=60
    ):
        try:
            # 1. Fetch info
            ...
            for dep in deps:
                ...
                neo4j_client.add_package_dependency(
                    "maven", child_name, "latest", "maven", pkg_name, "latest"
                )

            # 3. Write CVE Connection
            neo4j_client.mark_cve_affected(
                cve_id, 0.0, "maven", pkg_name, pkg.get("versions_affected", [])
            )
            ...
            logger.info(
                f"Resolved {len(deps)} dependents for Maven artifact {pkg_name}"
            )
