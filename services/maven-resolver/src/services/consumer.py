...
class MavenCveConsumer(KafkaConsumerBase):
    def __init__(self, bootstrap_servers: str, group_id: str, resolver_service):
        super().__init__(bootstrap_servers, group_id, topics=["resolver.maven"])
        self.resolver_service = resolver_service

    async def process_message(self, message_data: Dict[str, Any]):
        """
        Process a message routed to the Maven resolver.
        """
        try:
            cve_id = message_data.get("cve_id")
            target_package = message_data.get("target_package")
            
            logger.info(f"Received Maven CVE for resolution: {cve_id} (Package: {target_package})")
            ...
            logger.error(f"Failed to process Maven coordination message: {e}")
