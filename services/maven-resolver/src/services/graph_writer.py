...
class GraphWriter:
    ...
    async def create_dependency(self, parent_name: str, child_name: str, ecosystem: str, version_req: str, scope: str = "compile"):
        """
        Create a DEPENDS_ON relationship between packages, including Maven scope.
        """
        query = """
        ...
        SET r.version_requirement = $version_req, r.scope = $scope, r.updated_at = timestamp()
        """
        async with self.driver.session() as session:
            await session.run(query, ecosystem=ecosystem, parent_name=parent_name, child_name=child_name, version_req=version_req, scope=scope)
