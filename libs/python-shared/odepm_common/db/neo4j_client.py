...
class Neo4jClient:
    ...
    def add_package_dependency(
        ...
        with self.driver.session() as session:
            session.run(
                query,
                parent_eco=parent_ecosystem,
                parent_name=parent_name,
                parent_version=parent_version,
                child_eco=child_ecosystem,
                child_name=child_name,
                child_version_spec=child_version_spec,
                depth=depth,
            )

    def add_repository_usage(
        self, repo_url, repo_owner, package_ecosystem, package_name, version_spec
    ):
        query = """
        ...
        with self.driver.session() as session:
            session.run(
                ...
                package_eco=package_ecosystem,
                package_name=package_name,
                version_spec=version_spec,
            )

    def mark_cve_affected(
        self, cve_id, cvss_score, package_ecosystem, package_name, versions_affected
    ):
        query = """
