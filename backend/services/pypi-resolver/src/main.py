...
from api_clients import PyPIClient, LibrariesIoClient
...
async def process_package(cve_id: str, pkg: dict):
    ...
    if redis_client.check_rate_limit(f"rate:pypi:{pkg_name}", limit=100, window_seconds=60):
        try:
            ...
            for dep in deps:
                ...
                neo4j_client.add_package_dependency("pypi", child_name, "latest", "pypi", pkg_name, "latest")
            
            # 4. Write Cve connection
            neo4j_client.mark_cve_affected(cve_id, 0.0, "pypi", pkg_name, pkg.get("versions_affected", []))
